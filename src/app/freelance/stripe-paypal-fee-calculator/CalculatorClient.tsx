...
export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
    ...
    if (urlSavedId) {
      getSharedCalculatorAction(urlSavedId).then(data => {
        if (data && data.input_state) {
          ...
          if (state.invoiceAmount !== undefined) store.setInvoiceAmount(state.invoiceAmount as any)
          if (state.sourceCurrency !== undefined) store.setSourceCurrency(state.sourceCurrency as any)
          if (state.targetCurrency !== undefined) store.setTargetCurrency(state.targetCurrency as any)
          if (state.midMarketRate !== undefined) store.setMidMarketRate(state.midMarketRate as any)
          if (state.annualInvoices !== undefined) store.setAnnualInvoices(state.annualInvoices as any)
  ...
  const shareUrl = savedScenarioId 
    ? `${baseUrl}/freelance/cross-border-fx-impact-calculator?savedId=${savedScenarioId}`
    : `${baseUrl}/freelance/cross-border-fx-impact-calculator`
