...
export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
    ...
    if (urlSavedId) {
      getSharedCalculatorAction(urlSavedId).then(data => {
        if (data && data.input_state) {
          ...
          if (state.weeklyUnbilledScopeCreep !== undefined) store.setWeeklyUnbilledScopeCreep(state.weeklyUnbilledScopeCreep as any)
          if (state.weeklyAdminTimeTracking !== undefined) store.setWeeklyAdminTimeTracking(state.weeklyAdminTimeTracking as any)
          if (state.hourlyRate !== undefined) store.setHourlyRate(state.hourlyRate as any)
          if (state.softwareCostPerMonth !== undefined) store.setSoftwareCostPerMonth(state.softwareCostPerMonth as any)
  ...
  const shareUrl = savedScenarioId 
    ? `${baseUrl}/freelance/time-tracking-roi-calculator?savedId=${savedScenarioId}`
    : `${baseUrl}/freelance/time-tracking-roi-calculator`
