    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch relative">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-muted/30 rounded-xl p-4 border border-border/50">
            <div className="flex w-full h-3 rounded-full overflow-hidden mb-3">
              ...
              <div style={{ width: `${100 - store.salaryRatio}%` }} className="bg-blue-500 h-full"></div>
            </div>
            <div className="flex justify-between text-sm font-semibold">
              <div className="text-foreground/80">W-2 Salary: ${Math.round(metrics.sCorpW2Salary).toLocaleString()}</div>
              <div className="text-blue-700">Distribution: ${Math.round(metrics.sCorpDistribution).toLocaleString()}</div>
            </div>
          </div>
        </div>
      ...
      <div className="lg:col-span-12 xl:col-span-7 flex flex-col gap-6">
        ...
        <div className={`mt-2 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-lg border relative overflow-hidden ${
          metrics.isWorthIt ? 'bg-blue-600 border-blue-500 text-white' : 'bg-slate-100 border-slate-300 text-foreground/80'
        }`}>
          {metrics.isWorthIt && (
          ...
          <div>
            ...
            <p className={`text-sm ${metrics.isWorthIt ? 'text-blue-100' : 'text-muted-foreground'}`}>
          ...
          <div className="text-right">
            ...
            <span className={`text-sm ml-1 ${metrics.isWorthIt ? 'text-blue-100' : 'text-muted-foreground'}`}>/ year</span>
          </div>
        </div>
        ...
        <div className="bg-primary/5 text-foreground/80 p-5 rounded-2xl text-sm border border-primary/20 mt-4 shadow-sm">
          ...
          {metrics.isWorthIt ? (
            <span className="block mt-2 text-blue-700 font-bold">✓ Since your gross savings (${Math.round(metrics.annualTaxSavings).toLocaleString()}) exceed these admin costs, an S-Corp election is likely highly profitable for you.</span>
          ) : (
