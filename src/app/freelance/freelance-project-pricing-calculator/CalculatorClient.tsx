...
export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
    ...
    if (urlSavedId) {
      getSharedCalculatorAction(urlSavedId).then(data => {
        if (data && data.input_state) {
          ...
          if (state.estimatedHours !== undefined) store.setEstimatedHours(state.estimatedHours as any)
          if (state.targetHourlyRate !== undefined) store.setTargetHourlyRate(state.targetHourlyRate as any)
          if (state.commBufferPct !== undefined) store.setCommBufferPct(state.commBufferPct as any)
          if (state.riskLevel !== undefined) store.setRiskLevel(state.riskLevel as any)
          if (state.clientROI !== undefined) store.setClientROI(state.clientROI as any)
          if (state.roiSharePct !== undefined) store.setRoiSharePct(state.roiSharePct as any)
  ...
  const shareUrl = savedScenarioId 
    ? `${baseUrl}/freelance/project-flat-fee-quoter?savedId=${savedScenarioId}`
    : `${baseUrl}/freelance/project-flat-fee-quoter`
