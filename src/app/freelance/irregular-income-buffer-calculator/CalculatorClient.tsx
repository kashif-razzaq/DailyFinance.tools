...
export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
    ...
    if (urlSavedId) {
      getSharedCalculatorAction(urlSavedId).then(data => {
        if (data && data.input_state) {
          ...
          if (state.essentialLiving !== undefined) store.setEssentialLiving(state.essentialLiving as any)
          if (state.bizOverhead !== undefined) store.setBizOverhead(state.bizOverhead as any)
          if (state.incomeHistory !== undefined) {
            store.setIncomeHistory(state.incomeHistory as any)
            setIncomeInput((state.incomeHistory as any)?.join(', '))
          }
          if (state.topClientShare !== undefined) store.setTopClientShare(state.topClientShare as any)
          if (state.avgDSO !== undefined) store.setAvgDSO(state.avgDSO as any)
          if (state.currentBuffer !== undefined) store.setCurrentBuffer(state.currentBuffer as any)
          if (state.targetMonthsToBuild !== undefined) store.setTargetMonthsToBuild(state.targetMonthsToBuild as any)
  ...
  const shareUrl = savedScenarioId 
    ? `${baseUrl}/freelance/irregular-income-buffer-calculator?savedId=${savedScenarioId}`
    : `${baseUrl}/freelance/irregular-income-buffer-calculator`
