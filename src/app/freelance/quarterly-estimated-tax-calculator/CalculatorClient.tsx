...
export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
    ...
    if (urlSavedId) {
      getSharedCalculatorAction(urlSavedId).then(data => {
        if (data && data.input_state) {
          ...
          if (state.netIncome !== undefined) store.setNetIncome(state.netIncome as any)
          if (state.stateTaxRate !== undefined) store.setStateTaxRate(state.stateTaxRate as any)
          if (state.claimsQBI !== undefined) store.setClaimsQBI(state.claimsQBI as any)
  ...
  const shareUrl = savedScenarioId 
    ? `${baseUrl}/freelance/quarterly-estimated-taxes-calculator?savedId=${savedScenarioId}`
    : `${baseUrl}/freelance/quarterly-estimated-taxes-calculator`
