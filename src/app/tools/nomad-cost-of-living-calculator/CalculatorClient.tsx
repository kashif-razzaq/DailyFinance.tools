...
export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
    ...
    if (urlSavedId) {
      getSharedCalculatorAction(urlSavedId).then(data => {
        if (data && data.input_state) {
          ...
          if (state.currentMonthlyExpenses !== undefined) store.setCurrentMonthlyExpenses(state.currentMonthlyExpenses)
          if (state.targetColReductionPct !== undefined) store.setTargetColReductionPct(state.targetColReductionPct)
          if (state.relocationCost !== undefined) store.setRelocationCost(state.relocationCost)
          if (state.currentSavings !== undefined) store.setCurrentSavings(state.currentSavings)
          if (state.monthlyIncome !== undefined) store.setMonthlyIncome(state.monthlyIncome)
  ...
  const shareUrl = savedScenarioId 
    ? `${baseUrl}/tools/nomad-cost-of-living-calculator?savedId=${savedScenarioId}`
    : `${baseUrl}/tools/nomad-cost-of-living-calculator`
        ...
        <div className="bg-card border rounded-2xl p-6 flex flex-col sm:flex-row gap-3 mt-auto shadow-sm">
          ...
          <div className="flex-1 flex gap-3">
            <div className="flex-1">
              <ExportEngine 
                data={exportData} 
                filename="NomadGeoArbitrage" 
                isPro={isPro}
                onRequirePro={() => setShowProModal(true)}
              />
            </div>
            <div className="flex-1">
            <ShareCalculatorModal url={shareUrl} slug="nomad-cost-of-living-calculator" isPro={isPro}>
