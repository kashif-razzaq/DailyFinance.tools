...
import { useMortgageAmortizationStore } from '@/store/mortgage_amortization.store'
...
export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
  ...
  const totalInterestStandard = (standardPayment * n) - P;

  // Calculate with extra payment
  let balance = P;
  let totalInterestWithExtra = 0;
  const actualPayment = standardPayment + store.extraMonthlyPayment;
  let months = 0;
  ...
  const metrics = {
    standardPayment, totalInterestStandard, totalInterestWithExtra, interestSaved
  };
  ...
  const shareUrl = savedScenarioId
    ? `${baseUrl}/real-estate/mortgage-amortization-calculator?savedId=${savedScenarioId}`
    : `${baseUrl}/real-estate/mortgage-amortization-calculator`
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch relative">
      ...
      <div className="lg:col-span-12 xl:col-span-8 flex flex-col gap-6">
        <div className="bg-card border shadow-sm rounded-2xl p-6 md:p-8 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Standard Monthly P&I</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.standardPayment).toLocaleString()}
              </p>
            </div>
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Total Interest (Standard)</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.totalInterestStandard).toLocaleString()}
              </p>
            </div>
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Total Interest (With Extra Payments)</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.totalInterestWithExtra).toLocaleString()}
              </p>
            </div>
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Total Interest Saved</h4>
              <p className="text-2xl font-bold text-foreground">
        ...
        <CalculatorActions
          ...
          exportFilename="mortgage-amortization-calculator"
