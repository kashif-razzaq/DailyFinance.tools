...
import { useCreditCardPayoffStore } from '@/store/credit_card_payoff.store'
import { Target, DollarSign, Activity } from 'lucide-react'

export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
  const dummyIsPro = isPro;
  const store = useCreditCardPayoffStore()
  ...
  const P = store.balance;
  const r = store.interestRate / 100 / 12;
  const pmt = store.monthlyPayment;
  ...
  if (pmt <= P * r) {
      ...
      while (balance > 0 && monthsToPayOff < 1200) {
  ...
  const metrics = {
    monthsToPayOff, totalInterest, totalPaid
  };
  ...
  const shareUrl = savedScenarioId
    ? `${baseUrl}/personal-wealth/credit-card-payoff-calculator?savedId=${savedScenarioId}`
    : `${baseUrl}/personal-wealth/credit-card-payoff-calculator`
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch relative">
      ...
      <div className="lg:col-span-12 xl:col-span-4 bg-card border shadow-sm rounded-2xl p-6 md:p-8 flex flex-col space-y-8">
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-sm font-semibold text-foreground">Credit Card Balance</label>
            <div className="relative">
              ...
              <Input
                type="number"
                value={store.balance === 0 ? '' : store.balance}
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <Input
                type="number"
                value={store.monthlyPayment === 0 ? '' : store.monthlyPayment}
                onChange={(e) => store.setMonthlyPayment(Number(e.target.value))}
                className="pl-9 bg-muted/50 text-lg font-bold"
              />
            </div>
          </div>
      ...
      <div className="lg:col-span-12 xl:col-span-8 flex flex-col gap-6">
        <div className="bg-card border shadow-sm rounded-2xl p-6 md:p-8 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Months to Pay Off</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.monthsToPayOff).toLocaleString()}
              </p>
            </div>
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Total Interest Paid</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.totalInterest).toLocaleString()}
              </p>
            </div>
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Total Paid</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.totalPaid).toLocaleString()}
              </p>
            </div>
          </div>
        </div>
        ...
        <CalculatorActions
          ...
          exportFilename="credit-card-payoff-calculator"
