...
import { useHsaTripleTaxAdvantageStore } from '@/store/hsa_triple_tax_advantage.store'
import { Target, DollarSign, Activity } from 'lucide-react'

export function CalculatorClient({ isPro = false }: { isPro?: boolean }) {
  const dummyIsPro = isPro;
  const store = useHsaTripleTaxAdvantageStore()
  ...
  const PMT = store.annualContribution;
  const r = store.annualReturnRate / 100;
  const t = store.yearsToInvest;

  const futureValue = r > 0 ? PMT * ( (Math.pow(1 + r, t) - 1) / r ) : PMT * t;
  const totalContributions = PMT * t;
  const taxSavingsNow = totalContributions * (store.marginalTaxRate / 100);
  const taxFreeGrowth = futureValue - totalContributions;


  const metrics = {
    totalContributions, taxSavingsNow, futureValue, taxFreeGrowth
  };
  ...
  const shareUrl = savedScenarioId
    ? `${baseUrl}/personal-wealth/hsa-triple-tax-advantage-calculator?savedId=${savedScenarioId}`
    : `${baseUrl}/personal-wealth/hsa-triple-tax-advantage-calculator`
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch relative">
      ...
      <div className="lg:col-span-12 xl:col-span-4 bg-card border shadow-sm rounded-2xl p-6 md:p-8 flex flex-col space-y-8">
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-sm font-semibold text-foreground">Annual Contribution</label>
            <div className="relative">
              ...
              <Input
                type="number"
                value={store.annualContribution === 0 ? '' : store.annualContribution}
                onChange={(e) => store.setAnnualContribution(Number(e.target.value))}
                className="pl-9 bg-muted/50 text-lg font-bold"
              />
            </div>
          </div>
          <div className="space-y-3">
            <label className="text-sm font-semibold text-foreground">Years to Invest</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-semibold"><DollarSign className="w-4 h-4"/></span>
              <Input
                type="number"
                value={store.yearsToInvest === 0 ? '' : store.yearsToInvest}
                onChange={(e) => store.setYearsToInvest(Number(e.target.value))}
                className="pl-9 bg-muted/50 text-lg font-bold"
              />
            </div>
          </div>
          <div className="space-y-3">
            <label className="text-sm font-semibold text-foreground">Expected Annual Return (%)</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-semibold"><DollarSign className="w-4 h-4"/></span>
              <Input
                type="number"
      ...
      <div className="lg:col-span-12 xl:col-span-8 flex flex-col gap-6">
        <div className="bg-card border shadow-sm rounded-2xl p-6 md:p-8 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Total Contributions</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.totalContributions).toLocaleString()}
              </p>
            </div>
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Tax Savings on Contributions</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.taxSavingsNow).toLocaleString()}
              </p>
            </div>
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Future HSA Balance</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.futureValue).toLocaleString()}
              </p>
            </div>
            <div className="border bg-card rounded-xl p-5">
              <h4 className="font-bold text-foreground mb-2 text-sm text-muted-foreground">Tax-Free Growth (Interest)</h4>
              <p className="text-2xl font-bold text-foreground">
                {Math.round(metrics.taxFreeGrowth).toLocaleString()}
              </p>
            </div>
          </div>
        </div>
        ...
        <CalculatorActions
          ...
          exportFilename="hsa-triple-tax-advantage-calculator"
